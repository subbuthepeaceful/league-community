...
describe LoginsController do
  describe "route generation" do
    it "should route the new logins action correctly" do
      route_for(:controller => 'logins', :action => 'new').should == "/login"
    end
    it "should route the create logins correctly" do
      route_for(:controller => 'logins', :action => 'create').should == "/login"
    end
    it "should route the destroy logins action correctly" do
      route_for(:controller => 'logins', :action => 'destroy').should == "/logout"
    end
  end
  
  describe "route recognition" do
    it "should generate params from GET /login correctly" do
      params_from(:get, '/login').should == {:controller => 'logins', :action => 'new'}
    end
    it "should generate params from POST /login correctly" do
      params_from(:post, '/login').should == {:controller => 'logins', :action => 'create'}
    end
    it "should generate params from DELETE /login correctly" do
      params_from(:delete, '/logout').should == {:controller => 'logins', :action => 'destroy'}
    end
  end
